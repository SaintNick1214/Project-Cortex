...
async def validate_agent_registration(client, graph_adapter):
    ...
    if len(nodes) == 0:
        print(f"   ❌ FAILED: Agent node not found in graph!")
        return False
...
async def validate_memory_space_sync(client, graph_adapter):
    ...
    if len(nodes) == 0:
        print(f"   ❌ FAILED: MemorySpace node not found in graph!")
        return False
...
async def validate_conversation_and_memory_sync(client, graph_adapter):
    ...
    if len(conv_nodes) == 0:
        print(f"   ❌ FAILED: Conversation node not found!")
        return False
    ...
    if edge_query.count == 0 or edge_query.records[0]["edgeCount"] == 0:
        print(f"   ⚠️  WARNING: REFERENCES edge not found (might be async)")
    else:
...
async def validate_fact_storage_sync(client, graph_adapter):
    ...
    if len(fact_nodes) == 0:
        print(f"   ❌ FAILED: Fact node not found in graph!")
        return False
...
async def validate_mutable_kv_sync(client, graph_adapter):
    ...
    if len(nodes) == 0:
        print(f"   ⚠️  WARNING: MutableEntry node not found (might need syncToGraph=True in implementation)")
...
async def validate_traverse_operation(client, graph_adapter):
    ...
    if len(connected) == 0:
        print(f"   ⚠️  WARNING: No connected nodes found (check relationship creation)")
