...
async function main() {
  ...
  console.log(
    `   Multiple chunks created: ${chunks4.length > 1 ? "✅" : "❌"}`,
  );
  console.log();
  ...
  console.log(
    `✅ Long content (15000): ${shouldChunk1 ? "Should chunk ✅" : "Should not chunk ❌"}`,
  );
  console.log(
    `✅ Short content (5000): ${shouldChunk2 ? "Should chunk ❌" : "Should not chunk ✅"}`,
  );
  console.log();
  ...
  const allHaveMetadata = chunks7.every(
    (chunk) =>
  ...
  console.log(
    `   All chunks have proper metadata: ${allHaveMetadata ? "Yes" : "No"}`,
  );
  console.log();
  ...
  assert(
    overlapChunks.length >= 3,
    "Should create multiple chunks with overlap",
  );
  assert(overlapChunks[0].content === "0123", "First chunk should be '0123'");
  assert(
    overlapChunks[1].content.startsWith("3"),
    "Second chunk should start with overlap",
  );
  assert(
    overlapChunks[0].metadata.hasOverlap === false,
    "First chunk should not have overlap flag",
  );
  assert(
    overlapChunks[1].metadata.hasOverlap === true,
  ...
  assert(
    emptyChunks[0].content === "",
    "Empty chunk content should be empty string",
  );
  ...
  metaChunks.forEach((chunk, idx) => {
    ...
    assert(
      ...
      "metadata.chunkIndex should match",
    );
    assert(
      typeof chunk.metadata.totalChunks === "number",
      "metadata.totalChunks should be number",
    );
    assert(
      typeof chunk.metadata.hasOverlap === "boolean",
      "metadata.hasOverlap should be boolean",
    );
  });
