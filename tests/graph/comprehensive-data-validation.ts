...
async function testWithDatabase(name: string, config: any) {
  ...
  try {
    ...
    assert(convNode.properties.conversationId === conv.conversationId, "Conversation ID should match");
    ...
    const memNode = memNodes.find(n => n.properties.memoryId === memory.memoryId);
    assert(memNode !== undefined, "Should find memory node in graph");
    assert(memNode!.properties.content === "Test memory content", "Memory content should match");
    assert((memNode!.properties.importance as number) === 80, "Memory importance should match");
    ...
    assert(factNode!.properties.fact === "User prefers dark mode", "Fact content should match");
    ...
    const ctxNode = ctxNodes.find(n => n.properties.contextId === context.contextId);
    assert(ctxNode !== undefined, "Should find context node in graph");
    assert(ctxNode!.properties.purpose === "Test context", "Context purpose should match");
    ...
    const userNode = userNodes.find(n => 
    ...
    if (userNode) {
      ...
      console.log(`  ⚠️  Users API doesn't expose syncToGraph - uses Immutable API internally\n`);
    }
    ...
    if (spaceNodeCount > 0) {
      ...
      const spaceNode = spaceNodes.find(n => n.properties.memorySpaceId === space.memorySpaceId);
      if (spaceNode) {
        console.log(`  ✅ MemorySpace synced to graph\n`);
      } else {
        console.log(`  ⚠️  MemorySpace node exists but properties don't match\n`);
      }
    } else {
      console.log(`  ❌ BUG: MemorySpaces API registered but NO graph node created\n`);
    }
    ...
    if (immNodeCount > 0) {
      ...
      const immNode = immNodes.find(n => n.properties.immutableType === "kb-article");
      if (immNode) {
      ...
      console.log(`  ❌ BUG: Immutable data stored but NO graph node created\n`);
    }
    ...
    assert(totalEdges > 0, `Should have relationships in graph, got ${totalEdges}`);
    console.log(`  ✅ Found ${totalEdges} relationships in graph`);
    
    // Check specific relationship types exist
    const query = await adapter.query("MATCH ()-[r]->() RETURN DISTINCT type(r) as type");
    console.log(`  ✅ Relationship types: ${query.records.map(r => r.type).join(", ")}\n`);
...
async function main() {
  console.log("╔═══════════════════════════════════════════════════════════════╗");
  console.log("║   Comprehensive Data Validation - All Graph-Integrated APIs  ║");
  console.log("╚═══════════════════════════════════════════════════════════════╝");
