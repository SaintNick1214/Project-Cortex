...
async function testDatabase(name: string, config: any) {
  ...
  try {
    ...
    console.log(`  ✅ traverse() → found ${traversed.length} nodes (${traversed.length >= 2 ? 'connected' : 'no connections found'})`);
    ...
    try {
      ...
      if ((error as Error).message.includes("shortestPath")) {
        console.log("  ⚠️  findPath() → not supported in this database (Memgraph limitation)");
      } else {
        throw error;
      }
    }
    ...
    console.log(`  ✅ batchWrite() → created ${nodesAfter - nodesBefore} nodes`);
    console.log("");
...
async function main() {
  console.log("╔═══════════════════════════════════════════════════════════════╗");
  console.log("║     Comprehensive Graph DB API Test (Neo4j + Memgraph)       ║");
  console.log("╚═══════════════════════════════════════════════════════════════╝");
  ...
  try {
    ...
    console.log("   • Connection management (connect, disconnect, isConnected)");
    console.log("   • Node operations (create, get, update, delete, find, count)");
    ...
    console.log("   • Utility operations (countNodes, countEdges, clearDatabase)");
    console.log("");
