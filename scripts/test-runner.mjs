...
function runTests(mode) {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const jestPath = resolve(projectRoot, 'node_modules', 'jest', 'bin', 'jest.js');
...
async function main() {
  ...
  console.log(`   Local config: ${hasLocalConfig ? '✅ Found' : '❌ Not found'}`);
  console.log(`   Managed config: ${hasManagedConfig ? '✅ Found' : '❌ Not found'}`);
  console.log(`   Test mode: ${testMode}\n`);

  // Handle explicit test mode
  if (testMode === 'local') {
    if (!hasLocalConfig) {
      console.error('❌ LOCAL test mode requested but LOCAL_CONVEX_URL not configured');
      process.exit(1);
    }
  ...
  if (testMode === 'managed') {
    if (!hasManagedConfig) {
      console.error('❌ MANAGED test mode requested but CONVEX_URL not configured');
      process.exit(1);
    }
  ...
  if (testMode === 'auto') {
    ...
    if (testSuites.length === 2) {
      ...
      console.log('   Tests will run against both local AND managed environments\n');
    }
