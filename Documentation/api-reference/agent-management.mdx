...
> ⚠️ **IMPORTANT:** The Agent Management API is being superseded by [Memory Space Operations](./memory-space-operations). While agent-based terminology remains supported for backwards compatibility, new applications should use memory spaces.
>
> **Migration Path:**
>
> - `agentId` → `memorySpaceId` (fundamental change)
> - `cortex.agents.*` → `cortex.memorySpaces.*` (new API)
> - See [Memory Spaces Guide](../02-core-features/01-memory-spaces) for details
...
- **[User Operations API](./user-operations)** - User profile management
- **[Memory Operations API](./memory-operations)** - Memory CRUD
