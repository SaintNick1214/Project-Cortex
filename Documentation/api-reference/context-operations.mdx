...
`options` parameter with `syncToGraph` support for graph database integration.
...
- [Context Chains Guide](/core-features/context-chains#creating-a-root-context)
...
`options` parameter with `syncToGraph` support.
...
- [Updating Context](/core-features/context-chains#updating-context)
...
<APITable
  parameters={[
    ...
    { name: "syncToGraph", type: "boolean", required: false, default: "false", description: "Delete from graph with orphan cleanup" },
  ]}
/>
...
`syncToGraph` option enables graph deletion with sophisticated orphan cleanup.
...
- [Querying Contexts](/core-features/context-chains#querying-contexts)
...
<QuickNavItem
  ...
  description="Graph database integration (NEW in v0.7.0)"
