...
Session policies defined here configure the behavior enforced by the [Sessions API](./sessions-operations). The Governance API defines the _rules_, while the Sessions API _executes_ them.
...
- **[Immutable Store API](./immutable-store-api)** - Versioned shared data
- **[Mutable Store API](./mutable-store-api)** - Live shared data
- **[Memory Operations API](./memory-operations)** - Vector layer
- **[Conversation Operations API](./conversation-operations)** - Private conversations
