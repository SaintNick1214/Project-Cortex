...
> **AI SDK Compatibility**: v3.0.0, v4.0.0, v5.0.0, v6.0.0+  
...
> 💡 **Note:** For async initialization (e.g., when Convex client needs to be created asynchronously), use `createCortexMemoryAsync` instead of `createCortexMemory`. See the [API Reference](./api-reference) for details.
...
- [Memory Operations](/api-reference/memory-operations) - remember() and rememberStream() API
...
- [Memory Spaces](/core-features/memory-spaces) - Multi-tenancy
- [Hive Mode](/core-features/hive-mode) - Cross-application memory
