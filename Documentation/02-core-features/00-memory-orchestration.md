...
│   │ (user↔agent) │  │ (KB, audit)  │  │  (live data) │     │
│   └──────────────┘  └──────────────┘  └──────────────┘     │
└───────────────────────────┬─────────────────────────────────┘
                            │ References
                            ▼
┌─────────────────────────────────────────────────────────────┐
│       Layer 2: Vector Index (Semantic Search)               │
...
└───────────────────────────┬─────────────────────────────────┘
                            ...
                            ▼
┌─────────────────────────────────────────────────────────────┐
│       Layer 4: Convenience API (Orchestration)              │
│   cortex.memory.remember() → All layers coordinated         │
...
│   Multi-hop traversal and entity relationships              │
└─────────────────────────────────────────────────────────────┘
```
...
remember() call
      ...
      ├─ 1. Validates inputs
      │
      ├─ 2. Auto-registers memory space (if new)
      │
      ├─ 3. Auto-creates user profile (if new)
      │
      ├─ 4. Stores in ACID conversation (Layer 1)
      │     └─ User message + Agent response
      │
      ├─ 5. Creates vector memories (Layer 2)
      │     └─ With optional embeddings
      │
      ├─ 6. Extracts & stores facts (Layer 3)
      │     └─ With deduplication and belief revision
      │
      └─ 7. Syncs to graph (if configured)
...
recall() call
      │
      ├─ 1. Searches vector memories (Layer 2)
      │     └─ Semantic search with optional embedding
      │
      ├─ 2. Searches facts directly (Layer 3)
      │     └─ Facts as primary source, not just enrichment
      │
      ├─ 3. Queries graph relationships (if configured)
      │     └─ Discovers related context via entity connections
      │
      ├─ 4. Merges, deduplicates, and ranks results
      │     └─ Multi-signal scoring algorithm
      │
      └─ 5. Formats for LLM injection
...
```typescript
// ✅ For most use cases, use cortex.memory.*
...
```typescript
// 💬 Direct conversation access (Layer 1a)
...
// 🧠 Direct fact operations (Layer 3)
await cortex.facts.revise(factParams);

// 📚 Shared knowledge (Layer 1b)
...
- **[Memory Spaces](./01-memory-spaces.md)** - Isolation boundaries for multi-tenant/multi-agent
- **[Semantic Search](./02-semantic-search.md)** - Deep dive into retrieval strategies
- **[Fact Extraction](./08-fact-extraction.md)** - Structured knowledge extraction
- **[Streaming Support](./12-streaming-support.md)** - Real-time AI integrations
