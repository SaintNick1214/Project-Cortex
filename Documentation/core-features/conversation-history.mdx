...
await cortex.conversations.addMessage(conv.id, {
  role: 'user',
  text: 'Hello!',
  userId: 'user-123',
});
...
```typescript
// User message
const msg = await cortex.conversations.addMessage(conv.id, {
  role: 'user',
...
```typescript
{
  ...
  messages: [
    { id: 'a2a-msg-001', from: 'finance-agent', to: 'hr-agent', text: '...' }
  ]
}
```
