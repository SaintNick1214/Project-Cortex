...
> 🚧 **PLANNED FEATURE**: The `cortex.analytics.*` API described in this document is planned for a future release. Currently, memory space statistics are available via `cortex.memorySpaces.getStats()`. See the [Memory Space Operations API](../03-api-reference/11-memory-space-operations) for available methods.
...
- **[Agent Memory](./memory-spaces)** - Vector layer fundamentals
- **[Conversation History](./conversation-history)** - ACID layer details
- **[Vector Embeddings](../04-architecture/04-vector-embeddings)** - Choosing embedding dimensions
