...
if (hasGraphConfig && hasDockerCompose) {
  ...
  const dockerRunning = await new Promise((resolve) => {
    ...
    dockerCheck.on('close', (code) => {
      if (code !== 0 && stderr.includes('Cannot connect to the Docker daemon')) {
  ...
  if (!dockerRunning) {
    ...
    const dockerUp = spawn('docker-compose', ['-f', 'docker-compose.graph.yml', 'up', '-d', '--remove-orphans'], {
      stdio: 'pipe',
    });
    ...
    dockerUp.on('close', (code) => {
      if (code === 0) {
        ...
        if (stdout.includes('Running') || stdout.includes('Started')) {
          ...
          if (process.env.NEO4J_URI?.includes('neo4j') || stdout.includes('neo4j')) {
        ...
        } else if (stderr.includes('already in use') || stderr.includes('Conflict')) {
          console.log('   ⚠️  Container name conflict (old stopped container)');
...
process.on('SIGINT', () => {
  ...
  if (hasGraphConfig && hasDockerCompose) {
    ...
    const dockerDown = spawn('docker-compose', ['-f', 'docker-compose.graph.yml', 'stop'], {
      stdio: 'pipe',
    });
